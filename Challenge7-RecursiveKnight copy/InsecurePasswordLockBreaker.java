//UIUC CS125 SPRING 2017 MP. File: InsecurePasswordLockBreaker.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2017-04-11T09:33:02-0500.466225000
//@author mapope2
public class InsecurePasswordLockBreaker {
	...
	public static char[] breakLock(InsecurePasswordLock lock) {
		...
		char[] attempt = new char[0];
		while (lock.open(attempt)==-1) {
			attempt = new char[attempt.length + 1];
		}
		
		for (int i=0;i<attempt.length;i++) {
			attempt[i] = 'A';
			while (lock.open(attempt)==i)
				attempt[i]++;
		}
		return attempt;
	}
